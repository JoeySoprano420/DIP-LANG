...
// Function to generate an .exe file (Simulated)
void generateExe(const std::string& code) {
    std::ofstream exeFile("output.exe");
    exeFile << "// Executable code generated from DIP-LANG\n";
    exeFile << "Executable Output:\n";
    exeFile << code << "\n";
    exeFile.close();
    std::cout << "Executable (.exe) generated successfully!" << std::endl;
}

// Function to generate an .ita file (Simulated)
void generateIta(const std::string& code) {
    std::ofstream itaFile("output.ita");
    itaFile << "// ITA file generated from DIP-LANG\n";
    itaFile << "ITA Output:\n";
    itaFile << code << "\n";
    itaFile.close();
    std::cout << "ITA file generated successfully!" << std::endl;
}
...
int main(int argc, char** argv) {
    ...
    std::string outputFormat = "exe"; // Default to .exe

    if (argc > 1) {
        outputFormat = argv[1];  // User specifies the output format
    }
    ...
    try {
        ...
        if (outputFormat == "exe") {
            generateExe(compiledOutput);
        } else if (outputFormat == "ita") {
            generateIta(compiledOutput);
        } else {
